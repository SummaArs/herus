...
|---|---|---|
| Everything | `./prove.sh` | 9 suites + 32 proof invariants + 74 system invariants |
...
1. **Write the failing test first.** `firmware/test/test_net.c` for the wire,
   `firmware/core/test_*.c` for algebra, Núcleo, voz/háptica, runtime e rig; `tools/test_interactionstudy.py` for pesquisa; `firmware/test/test_radio.c` for
