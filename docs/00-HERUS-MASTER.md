...
```bash
cd ~/Python/herus
./prove.sh                        # 11 suites, 34 proof invariants, all green
```

That covers the algebra, the opt-in bounded Nucleus predictor, controlled local voice/haptic contracts, the session-bound intent-confidence gateway, the authenticated Core↔Nucleus control envelope, the confirmed interaction runtime, the deterministic validation lab and telemetry gates, the preregistered study method, the protocol (crypto differentially tested against
