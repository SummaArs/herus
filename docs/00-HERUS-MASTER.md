...
```bash
cd ~/Python/herus
./prove.sh                        # 8 suites, 31 proof invariants, all green
```

That covers the algebra, the opt-in bounded Nucleus predictor, controlled local voice/haptic contracts, the confirmed interaction runtime, the deterministic validation lab and telemetry gates, the protocol (crypto differentially tested against
