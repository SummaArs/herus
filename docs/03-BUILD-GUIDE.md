...
2. Prove everything. One command, four suites, 26 invariants:
...
algebra (T1–T11), the protocol (crypto vs OpenSSL, ratchet, replay, forgery,
...
2. **This is already written.** `firmware/net/` and `firmware/port/` contain the
   whole stack, and `./prove.sh` verifies 26 invariants over it before you flash
