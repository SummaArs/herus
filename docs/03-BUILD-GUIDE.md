...
2. Prove everything. One command, thirty-four suites, 77 proof invariants:
...
2. **This is already written.** `firmware/net/` and `firmware/port/` contain the
   whole stack, and `./prove.sh` verifies 77 proof invariants over the full repository before you flash
...
[ ] Phase −1  collection Grand Finale, purpose-bound session gate, reservation-recovery oracle and boot-quarantine bridge pass; target backend/event/time/reset/UI evidence remains pending
...
[ ] Phase 4   each collection transition and reservation `PREPARED`/floor/`COMMITTED`/boot-quarantine path interrupted and recovered against its target backend
