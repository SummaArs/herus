...
2. Prove everything. One command, eight suites, 31 proof invariants:
...
algebra (T1–T11), the opt-in bounded Nucleus predictor, controlled voice/haptic contracts, the confirmed interaction runtime, deterministic adapter/telemetry gates, the protocol (crypto vs OpenSSL, ratchet, replay, forgery,
...
2. **This is already written.** `firmware/net/` and `firmware/port/` contain the
   whole stack, and `./prove.sh` verifies 31 proof invariants over it before you flash
