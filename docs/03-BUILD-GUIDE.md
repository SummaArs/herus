...
2. Prove everything. One command, thirty-two suites, 73 proof invariants:
...
[ ] Phase −1  collection Grand Finale and purpose-bound session gate pass; target backend/event/time/reset/UI evidence remains pending
...
[ ] Phase 4   each collection transition and session begin/consume/cancel path interrupted and recovered against its target backend
