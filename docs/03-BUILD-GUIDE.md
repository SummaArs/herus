...
2. Prove everything. One command, thirty-three suites, 75 proof invariants:
...
2. **This is already written.** `firmware/net/` and `firmware/port/` contain the
   whole stack, and `./prove.sh` verifies 75 proof invariants over the full repository before you flash
...
[ ] Phase −1  collection Grand Finale, purpose-bound session gate and reservation-recovery oracle pass; target backend/event/time/reset/UI evidence remains pending
...
[ ] Phase 4   each collection transition and reservation `PREPARED`/floor/`COMMITTED` path interrupted and recovered against its target backend
