...
2. Prove everything. One command, thirty-five suites, 79 proof invariants:
...
2. **This is already written.** `firmware/net/` and `firmware/port/` contain the
   whole stack, and `./prove.sh` verifies 79 proof invariants over the full repository before you flash
...
[ ] Phase 4   each collection transition and reservation `PREPARED`/floor/`COMMITTED`/boot-quarantine/final-chain path interrupted and recovered against its target backend
