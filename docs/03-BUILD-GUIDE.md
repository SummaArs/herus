...
2. Prove everything. One command, five suites, 27 code invariants:
...
algebra (T1–T11), the opt-in bounded Nucleus predictor, the protocol (crypto vs OpenSSL, ratchet, replay, forgery,
...
2. **This is already written.** `firmware/net/` and `firmware/port/` contain the
   whole stack, and `./prove.sh` verifies 27 code invariants over it before you flash
