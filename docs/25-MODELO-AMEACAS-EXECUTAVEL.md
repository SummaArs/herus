...
| TM-04 | Retenção sem consentimento, terceiro/sensível, rollback, duplicata, capacidade ou transação contraditória. | Captura física, política, autoridade humana, cofre/coleção autenticados, geração, revisão, conflito, capacidade fixa e commit estrito. | `test_memory_*`, `test_memory_collection.c` e `test_memory_finale.c`. | `MITIGATED_HOST` | NVS, raiz, power-loss, apagamento físico, endurance e backend multi-cartão no alvo não são provados. |
