...
A nomenclatura segue a existente — Core, Band, Lexicon, HCP, Weave, Echo, Vault, Beat ([00-HERUS-MASTER §3](../10-arquitetura/00-HERUS-MASTER.md)). Herald, Ladder, Drift, Aura e Keel entram nela, não ao lado dela.
...
| `memory_semantic_evidence` | a memória temporal com supersessão **já existe** ([68](../30-memoria/68-HERUS-MEMORIA-SEMANTICA-TEMPORAL-E-CONFLITOS.md)); a HSCA não a reescreve |
...
| OWL 2 / `SROIQ(D)` + SPARQL | `SROIQ(D)` é N2ExpTime-completo; um triplestore com SPARQL sub-milissegundo não roda em ESP32-S3, e exigir isso empurraria o produto de volta para compute externo — o oposto de [64](../30-memoria/64-HERUS-SOBERANIA-ON-WRIST-E-FRONTEIRA-CORE.md) |
...
- Não é medição de campo. Alcance, energia, ergonomia, RF, antena, bateria e latência real continuam pendentes da Fase 0 ([03-BUILD-GUIDE](../60-hardware/03-BUILD-GUIDE.md)).
