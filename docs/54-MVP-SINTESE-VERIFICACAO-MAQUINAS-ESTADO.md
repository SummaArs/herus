...
A transferência da garantia do modelo abstrato para uma implementação concreta exige uma relação de refinamento explícita. O mapa de estados deve ser total, o estado inicial concreto deve mapear para o estado inicial abstrato e cada transição concreta deve corresponder a uma transição abstrata. A implementação aceita mapas explícitos e totais de estados, entradas e ações, permitindo nomes concretos diferentes somente quando a correspondência mapeada for demonstrada. Um mapa omitido ou incompleto é `INVALID`; uma prova do modelo sem esse vínculo não é uma prova da implementação.
