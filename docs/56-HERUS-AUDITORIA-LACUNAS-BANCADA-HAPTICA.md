...
**Conclusão:** o contrato semântico, o adaptador host-portátil e o validador privado já existem e passaram a prova global 51/51; o port ESP32-S3 ainda não possui um caminho DRV2605L nem uma medição física. Nenhum gate físico foi aprovado.
...
| ESP32-S3 | HAL de oito funções, SPI/GPIO para SX1262, console e self-test de rádio | `firmware/port/hal.h`, `firmware/port/esp32s3/hal_esp32s3.c`, `app_main.c` | não há inicialização I²C, driver DRV2605L, leitura de status, calibração ou comando háptico |
...
A ordem não permite pular do encoder host para “vibração correta no pulso”. O primeiro alvo realista é demonstrar uma transação I²C observável, com o atuador em fixture de mesa e o firmware em estado seguro. Somente depois devem ser estudadas amplitude, duração, conforto ou reconhecimento.

## 5. Evidência host-only adicionada

O adaptador `firmware/core/haptic_adapter.{h,c}` e `test_haptic_adapter.c` passaram **17/17** invariantes em C11 estrito. A suíte cobre validação profile-bound antes do primeiro write, ordem `MODE → WAVEFORM → GO`, polling sem falso término, abort para standby, erro de bus, fault sem retry automático e rejeição de frame fragmentado. O validador de evidência `tools/haptic_bench_evidence_validate.py` passou **7/7** testes adversariais contra digest, conteúdo proibido, enum, I²C com erro, parada não observada e bloqueio sem motivo. Essas são provas host-only; não são resultados do DRV2605L.
