...
Depois, siga a Fase 0 já congelada no [guia de construção](../60-hardware/03-BUILD-GUIDE.md): conferir revisão/pin map, fechar a pilha de volume e operar os dois LilyGO T3-S3 a um metro antes de caminhar a rota urbana. Não reescreva a hipótese depois de ver o resultado: atualize o gate correspondente com arquivo de evidência, revisão, método e resultado `pass` ou `rejected`.
...
[3] [HERUS — Guia de construção e critérios de interrupção](../60-hardware/03-BUILD-GUIDE.md)
[4] [HERUS — Diálogo inteligente local](../50-inteligencia/14-DIALOGO-LLM-LOCAL.md)
[5] [HERUS — Laboratório de aceitação do modelo local](../50-inteligencia/15-LAB-ACEITACAO-MODELO.md)
