...
| Matriz de reboot e floor semântico | **12/12** |
| Redteam de reboot e floor | **8/8 mutantes mortos** |
...
A ponte `memory_reboot_boundary_bootstrap` executa a seguinte ordem: limpa a evidência volátil, recupera o floor de sessão por meio do bootstrap já existente, instala o mesmo floor no índice semântico quando ele é não nulo e bloqueia a fronteira se qualquer etapa falhar. O resultado informa o floor semântico importado, mas não retorna card, fato, sessão, nonce, propósito, uso ou autoridade.
...
A campanha removeu a limpeza semântica, a limpeza de argumentos inválidos, a validação de índice nulo, a declaração de scrubbed, a importação do floor, a rejeição de geração stale, a monotonicidade e a regra de índice vazio. Cada remoção fez a suíte falhar.
