...
#
# Twenty-six suites, each independently falsifiable:
...
#  15  threat-model executable host-evidence, target-pending and out-of-scope classifier
#  16  assurance    fail-closed cross-module composition and revocation precedence
#  17  capstone     dialogue, model, intent, trust and one-time-handoff attack chain
#  18  trust        explicit pairing, SAS, protected persistence and revocation
#  19  control-link authenticated Core/Nucleus envelope, expiry and replay protection
#  20  interaction  push-to-talk, confirmation, one-shot send and telemetry
#  21  validation   deterministic adapters and telemetry log gates
#  22  readiness    frozen hardware-evidence manifest and privacy/schema gate
#  23  study        preregistered plan, statistical gates and unsafe-send rejection
#  24  protocol     crypto vs OpenSSL, ratchet, framing, Weave, Beat, canonicality
#  25  radio        SX1262 command sequences against a recording mock bus
#  26  physical     RF, energy and the frame ledger, from tools/budget.py
#
...
banner "8/26 memory capture (physical session, expiry and transient discard)"
...
banner "9/26 memory extract (typed candidate, uncertainty and zero retention)"
...
banner "10/26 memory vault (explicit authority, AEAD and durable anti-rollback)"
...
banner "11/26 memory consolidation (bounded human review, conflict, recall and removal)"
...
banner "12/26 memory retrieval (typed local matching, ambiguity and zero authority)"
...
banner "13/26 memory retrieval presentation (one-shot status, uncertainty and zero authority)"
...
banner "14/26 memory Grand Finale (composed private-memory chain and zero model authority)"
...
banner "15/26 threat model (host evidence, target gaps and scope boundaries)"
...
banner "20/26 interaction (push-to-talk, confirmation and one-shot send)"
...
banner "21/26 validation lab (deterministic adapters and telemetry gates)"
...
banner "22/26 readiness manifest (frozen evidence and privacy gates)"
...
banner "23/26 preregistered study (frozen plan, gates and unsafe-send rejection)"
...
banner "24/26 protocol (crypto, ratchet, framing, Weave, Beat)"
( cd firmware && make net ) > /tmp/herus_b.log 2>&1 || FAIL=1
[ "$QUIET" = 0 ] && cat /tmp/herus_b.log
grep -q "FAIL" /tmp/herus_b.log && FAIL=1 || true

banner "25/26 radio driver (SX1262 command sequences, no hardware)"
...
banner "26/26 physical layer, energy and frame ledger"
