...
#
# Thirty-one suites, each independently falsifiable:
...
#  19  threat-model executable host-evidence, target-pending and out-of-scope classifier
#  20  assurance    fail-closed cross-module composition and revocation precedence
#  21  capstone     dialogue, model, intent, trust and one-time-handoff attack chain
#  22  trust        explicit pairing, SAS, protected persistence and revocation
#  23  control-link authenticated Core/Nucleus envelope, expiry and replay protection
#  24  interaction  push-to-talk, confirmation, one-shot send and telemetry
#  25  validation   deterministic adapters and telemetry log gates
#  26  readiness    frozen hardware-evidence manifest and privacy/schema gate
#  27  provenance   unsigned local inputs, declared components and fail-closed gates
#  28  study        preregistered plan, statistical gates and unsafe-send rejection
#  29  protocol     crypto vs OpenSSL, ratchet, framing, Weave, Beat, canonicality
#  30  radio        SX1262 command sequences against a recording mock bus
#  31  physical     RF, energy and the frame ledger, from tools/budget.py
#
...
banner "8/31 memory capture (physical session, expiry and transient discard)"
...
banner "9/31 memory extract (typed candidate, uncertainty and zero retention)"
...
banner "10/31 memory vault (explicit authority, AEAD and durable anti-rollback)"
...
banner "11/31 memory consolidation (bounded human review, conflict, recall and removal)"
...
banner "12/31 memory retrieval (typed local matching, ambiguity and zero authority)"
...
banner "13/31 memory retrieval presentation (one-shot status, uncertainty and zero authority)"
...
banner "14/31 memory Grand Finale (composed private-memory chain and zero model authority)"
...
banner "15/31 memory collection (bounded transactional multi-card persistence)"
...
banner "16/31 memory collection index (bounded private typed retrieval)"
...
banner "17/31 memory collection recovery (portable crash-state oracle)"
...
banner "18/31 memory collection Grand Finale (human authority, abstention and no fallback)"
...
banner "19/31 threat model (host evidence, target gaps and scope boundaries)"
...
banner "24/31 interaction (push-to-talk, confirmation and one-shot send)"
...
banner "25/31 validation lab (deterministic adapters and telemetry gates)"
...
banner "26/31 readiness manifest (frozen evidence and privacy gates)"
...
banner "27/31 local provenance manifest (unsigned inputs and pending supply-chain gates)"
...
banner "28/31 preregistered study (frozen plan, gates and unsafe-send rejection)"
...
banner "29/31 protocol (crypto, ratchet, framing, Weave, Beat)"
( cd firmware && make net ) > /tmp/herus_b.log 2>&1 || FAIL=1
[ "$QUIET" = 0 ] && cat /tmp/herus_b.log
grep -q "FAIL" /tmp/herus_b.log && FAIL=1 || true

banner "30/31 radio driver (SX1262 command sequences, no hardware)"
...
banner "31/31 physical layer, energy and frame ledger"
