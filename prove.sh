...
#
# Twenty suites, each independently falsifiable:
...
#  10  assurance    fail-closed cross-module composition and revocation precedence
#  11  capstone     dialogue, model, intent, trust and one-time-handoff attack chain
#  12  trust        explicit pairing, SAS, protected persistence and revocation
#  13  control-link authenticated Core/Nucleus envelope, expiry and replay protection
#  14  interaction  push-to-talk, confirmation, one-shot send and telemetry
#  15  validation   deterministic adapters and telemetry log gates
#  16  readiness    frozen hardware-evidence manifest and privacy/schema gate
#  17  study        preregistered plan, statistical gates and unsafe-send rejection
#  18  protocol     crypto vs OpenSSL, ratchet, framing, Weave, Beat, canonicality
#  19  radio        SX1262 command sequences against a recording mock bus
#  20  physical     RF, energy and the frame ledger, from tools/budget.py
#
...
banner "8/20 memory capture (physical session, expiry and transient discard)"
...
banner "9/20 memory extract (typed candidate, uncertainty and zero retention)"
...
banner "14/20 interaction (push-to-talk, confirmation and one-shot send)"
...
banner "15/20 validation lab (deterministic adapters and telemetry gates)"
...
banner "16/20 readiness manifest (frozen evidence and privacy gates)"
...
banner "17/20 preregistered study (frozen plan, gates and unsafe-send rejection)"
...
banner "18/20 protocol (crypto, ratchet, framing, Weave, Beat)"
( cd firmware && make net ) > /tmp/herus_b.log 2>&1 || FAIL=1
[ "$QUIET" = 0 ] && cat /tmp/herus_b.log
grep -q "FAIL" /tmp/herus_b.log && FAIL=1 || true

banner "19/20 radio driver (SX1262 command sequences, no hardware)"
...
banner "20/20 physical layer, energy and frame ledger"
